...
export const testResultsConversionWithDelete = () => describe("convertTestResults() integration tests with delete", async () => {
    let container: StartedTestContainer;
    const testResultsJson = JSON.parse(JSON.stringify(require("../resources/dynamodb-image-test-results.json")));
    const testResultsJsonWithTestTypes = JSON.parse(JSON.stringify(require("../resources/dynamodb-image-test-results-with-testtypes.json")));
    const testResultsJsonWithNoSystemNumber = JSON.parse(JSON.stringify(require("../resources/dynamodb-image-test-results-with-no-systemNumber.json")));
    const testResultsJsonWithoutTestTypes = JSON.parse(JSON.stringify(require("../resources/dynamodb-image-test-results-without-testtypes.json")));
    testResultsJson.testResultId.S = testResultsJson.testResultId.S + "-D";
    testResultsJson.systemNumber.S = testResultsJson.systemNumber.S + "-D";
    testResultsJsonWithTestTypes.testResultId.S = testResultsJsonWithTestTypes.testResultId.S + "-D";
    testResultsJsonWithTestTypes.systemNumber.S = testResultsJsonWithTestTypes.systemNumber.S + "-D";
    testResultsJsonWithNoSystemNumber.testResultId.S = testResultsJsonWithNoSystemNumber.testResultId.S + "-D";
    testResultsJsonWithoutTestTypes.testResultId.S = testResultsJsonWithoutTestTypes.testResultId.S + "-D";
    testResultsJsonWithoutTestTypes.systemNumber.S = testResultsJsonWithoutTestTypes.systemNumber.S + "-D";
    ...
    it("should correctly convert a DynamoDB event into Aurora rows", async () => {
        ...
        expect(vehicleResultSet.rows[0].system_number).toEqual("SYSTEM-NUMBER-5-D");
        ...
        expect(testResultSet.rows[0].testResultId).toEqual("TEST-RESULT-ID-5-D");
        ...
        expect(testResultSet.rows[0].secondaryCertificateNumber).toEqual("2ND-CERTIFICATE-NO");
        ...
        expect(testerResultSet.rows[0].email_address).toEqual("TESTER-EMAIL-ADDRESS-5");
        ...
        expect(lastUpdatedByResultSet.rows[0].identityId).toEqual("LAST-UPDATED-BY-ID-5");
        expect(lastUpdatedByResultSet.rows[0].name).toEqual("LAST-UPDATED-BY-NAME-5");
        ...
        expect(defectResultSet.rows[0].itemDescription).toEqual("ITEM-DESCRIPTION-5");
        ...
        expect(defectResultSet.rows[0].deficiencyText).toEqual("DEFICIENCY-TEXT-5");
        expect(defectResultSet.rows[0].itemNumber).toEqual(1);
        ...
        expect(customDefectResultSet.rows[customDefectLastIndex].referenceNumber).toEqual("def5");
        expect(customDefectResultSet.rows[customDefectLastIndex].defectName).toEqual("DEFECT-NAME-5");
        expect(customDefectResultSet.rows[customDefectLastIndex].defectNotes).toEqual("DEFECT-NOTES-5");
    });
    ...
    it("should correctly insert when unique attributes changed", async () => {
        ...
        expect(testResultSet.rows[0].secondaryCertificateNumber).toEqual("111111");
        expect(testResultSet.rows[0].testExpiryDate).toEqual(new Date("2022-01-02"));
        expect(testResultSet.rows[0].testAnniversaryDate).toEqual(new Date("2022-01-02"));
        expect((testResultSet.rows[0].testTypeStartTimestamp as Date).toISOString()).toEqual("2021-01-02T00:00:00.000Z");
        expect((testResultSet.rows[0].lastSeatbeltInstallationCheckDate as Date).toISOString()).toEqual("2021-01-02T00:00:00.000Z");
        expect(testResultSet.rows[0].seatbeltInstallationCheckDate).toEqual(0);
        expect(testResultSet.rows[0].testResult).toEqual("pass");
        expect(testResultSet.rows[0].reasonForAbandoning).toEqual("NEW-REASON-FOR-ABANDONING");
        expect(testResultSet.rows[0].additionalNotesRecorded).toEqual("NEW-ADDITIONAL-NOTES-RECORDED");
        ...
        expect(defectResultSet.rows.length).toEqual(1);
        expect(defectResultSet.rows[0].imNumber).toEqual(5);
        expect(defectResultSet.rows[0].imDescription).toEqual("IM-DESCRIPTION-5a");
        expect(defectResultSet.rows[0].itemNumber).toEqual(1);
        expect(defectResultSet.rows[0].itemDescription).toEqual("ITEM-DESCRIPTION-5");
        expect(defectResultSet.rows[0].deficiencyRef).toEqual("DEFICIENCY-REF-5");
        expect(defectResultSet.rows[0].deficiencyId).toEqual("a");
    ...
    it("A new Test Result with two TestTypes is inserted correctly", async () => {
        ...
        expect(vehicleResultSet.rows.length).toEqual(1);
        expect(vehicleResultSet.rows[0].system_number).toEqual("SYSTEM-NUMBER-3-D");
        ...
        expect(testResultSet.rows[0].testResultId).toEqual("TEST-RESULT-ID-3-D");
        expect(testResultSet.rows[0].testCode).toEqual("333");
        expect(testResultSet.rows[0].certificateNumber).toEqual("CERTIFICATE-NO");
        expect(testResultSet.rows[0].secondaryCertificateNumber).toEqual("2ND-CERTIFICATE-NO");
        ...
        expect(testResultSet.rows[1].testResultId).toEqual("TEST-RESULT-ID-3-D");
        ...
        expect(testDefectResultSet.rows[testDefectLastIndex].defect_id).toEqual(3);
        ...
        expect(defectResultSet.rows[0].itemNumber).toEqual(1);
        expect(defectResultSet.rows[0].itemDescription).toEqual("ITEM-DESCRIPTION-3");
        expect(defectResultSet.rows[0].deficiencyRef).toEqual("DEFICIENCY-REF-3");
        expect(defectResultSet.rows[0].deficiencyId).toEqual("a");
        expect(defectResultSet.rows[0].deficiencySubId).toEqual("mdclxvi");
        expect(defectResultSet.rows[0].deficiencyCategory).toEqual("advisory");
        expect(defectResultSet.rows[0].deficiencyText).toEqual("DEFICIENCY-TEXT-3");
        expect(defectResultSet.rows[0].itemNumber).toEqual(1);
        ...
        expect(customDefectResultSet.rows[customDefectLastIndex].test_result_id).toEqual(id);
        expect(customDefectResultSet.rows[customDefectLastIndex].referenceNumber).toEqual("def3");
        expect(customDefectResultSet.rows[customDefectLastIndex].defectName).toEqual("DEFECT-NAME-3");
        expect(customDefectResultSet.rows[customDefectLastIndex].defectNotes).toEqual("DEFECT-NOTES-3");
    ...
    it("A new Test Result with no TestTypes is inserted correctly", async () => {
        ...
        expect(vehicleResultSet.rows.length).toEqual(1);
        expect(vehicleResultSet.rows[0].system_number).toEqual("SYSTEM-NUMBER-4-D");
        ...
        expect(testResultSet.rows[0].testResultId).toEqual("TEST-RESULT-ID-4-D");
        ...
        expect(testerResultSet.rows[0].email_address).toEqual("TESTER-EMAIL-ADDRESS-4");
        ...
        expect(lastUpdatedByResultSet.rows.length).toEqual(1);
        expect(lastUpdatedByResultSet.rows[0].identityId).toEqual("LAST-UPDATED-BY-ID-4");
        expect(lastUpdatedByResultSet.rows[0].name).toEqual("LAST-UPDATED-BY-NAME-4");
    ...
    it("After all tests the database has all the expected data", async () => {
        const testResultResultSet = await executeSql(
            `SELECT id FROM test_result
                WHERE testResultId like '%-D';`
        );

        expect(testResultResultSet.rows.length).toEqual(4);
        ...
        expect(customDefectResultSet.rows.length).toEqual(2);
        ...
        expect(testDefectResultSet.rows.length).toEqual(2);
        ...
        expect(defectResultSet.rows.length).toEqual(2);
    });
});
