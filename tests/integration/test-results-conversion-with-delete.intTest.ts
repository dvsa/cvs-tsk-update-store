...
describe('convertTestResults() integration tests with delete', () => {
  ...
  it('should correctly convert a DynamoDB event into Aurora rows', async () => {
    ...
    expect(testResultSet.rows[0].testResultId).toBe('TEST-RESULT-ID-5-D');
  ...
  it('A new Test Result with two TestTypes is inserted correctly', async () => {
    ...
    expect(testResultSet.rows[0].testResultId).toBe('TEST-RESULT-ID-3-D');
    expect(testResultSet.rows[0].testCode).toBe('333');
    expect(testResultSet.rows[0].certificateNumber).toBe('CERTIFICATE-NO');
    expect(testResultSet.rows[0].secondaryCertificateNumber).toBe(
      '2ND-CERTIFICATE-NO',
    ...
    expect(testResultSet.rows[1].testResultId).toBe('TEST-RESULT-ID-3-D');
  ...
  it('A new Test Result with no TestTypes is inserted correctly', async () => {
    ...
    expect(testResultSet.rows[0].testResultId).toBe('TEST-RESULT-ID-4-D');
  ...
  it('After all tests the database has all the expected data', async () => {
    ...
    expect(testResultResultSet.rows).toHaveLength(4);
    ...
    expect(customDefectResultSet.rows).toHaveLength(2);
    ...
    expect(testDefectResultSet.rows).toHaveLength(2);
    ...
    expect(defectResultSet.rows).toHaveLength(2);
  });
});
