...
describe("convertTestResults() integration tests", () => {
    ...
    it("should correctly convert a DynamoDB event into Aurora rows", async () => {
        ...
        const vehicleResultSet = await executeSql(
            `SELECT \`system_number\`, \`vin\`, \`vrm_trm\`, \`trailer_id\`, \`createdAt\`, \`id\`
             ...
             WHERE \`vehicle\`.\`id\` IN (
                SELECT \`id\`
                FROM \`vehicle\`
                WHERE \`vehicle\`.\`system_number\` = "SYSTEM-NUMBER-5"
             )`
        );

        expect(vehicleResultSet.rows.length).toEqual(1);
        expect(vehicleResultSet.rows[0].system_number).toEqual("SYSTEM-NUMBER-5");
        ...
        expect(testResultSet.rows[0].testResultId).toEqual("TEST-RESULT-ID-5");
    ...
    it("should correctly convert a DynamoDB event into Aurora rows when a test type is updated", async () => {
    ...
    it("A new Test Result with two TestTypes is inserted correctly", async () => {
        ...
        expect(testResultSet.rows[0].testResultId).toEqual("TEST-RESULT-ID-3");
        expect(testResultSet.rows[0].testCode).toEqual("333");
        expect(testResultSet.rows[0].certificateNumber).toEqual("CERTIFICATE-NO");
        expect(testResultSet.rows[0].secondaryCertificateNumber).toEqual("2ND-CERTIFICATE-NO");
        ...
        expect(testResultSet.rows[1].testResultId).toEqual("TEST-RESULT-ID-3");
        ...
        expect(testDefectResultSet.rows[testDefectLastIndex].defect_id).toEqual(2);
    ...
    it("A new Test Result with no TestTypes is inserted correctly", async () => {
        ...
        const vehicleResultSet = await executeSql(
            `SELECT \`system_number\`, \`vin\`, \`vrm_trm\`, \`trailer_id\`, \`createdAt\`, \`id\`
             ...
             WHERE \`vehicle\`.\`id\` IN (
                SELECT \`id\`
                FROM \`vehicle\`
                WHERE \`vehicle\`.\`system_number\` = "SYSTEM-NUMBER-4"
             )`
        );

        expect(vehicleResultSet.rows.length).toEqual(1);
        expect(vehicleResultSet.rows[0].system_number).toEqual("SYSTEM-NUMBER-4");
        ...
        expect(testResultSet.rows[0].testResultId).toEqual("TEST-RESULT-ID-4");
