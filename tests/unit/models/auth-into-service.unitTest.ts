...
describe("parseAuthIntoService()", () => {
    it("should successfully parse a fully populated authIntoService image into an authIntoService object", () => {
        ...
        expect(techRecordDocument.techRecord![0].authIntoService?.dateAuthorised).toEqual("2020-02-02");
        expect(techRecordDocument.techRecord![0].authIntoService?.datePending).toEqual("2020-03-03");
        expect(techRecordDocument.techRecord![0].authIntoService?.dateReceived).toEqual("2020-04-04");
        expect(techRecordDocument.techRecord![0].authIntoService?.dateRejected).toEqual("2020-05-05");
    });
    ...
    it("should successfully parse an authIntoService image into an authIntoService object with some null properties", () => {
        ...
        expect(techRecordDocument.systemNumber).toEqual("SYSTEM-NUMBER-1");
        expect(techRecordDocument.techRecord![0].authIntoService?.cocIssueDate).toBeUndefined();
        expect(techRecordDocument.techRecord![0].authIntoService?.dateAuthorised).toEqual("2020-02-02");
        expect(techRecordDocument.techRecord![0].authIntoService?.datePending).toBeUndefined();
        expect(techRecordDocument.techRecord![0].authIntoService?.dateReceived).toEqual("2020-04-04");
        expect(techRecordDocument.techRecord![0].authIntoService?.dateRejected).toBeUndefined();
    });
